...
const Subscribe = (params: Props) => {
  ...
  const [coin, setCoin] = useState(Object.keys(coinNames)[0]);
  ...
  const writeButton = () => {
    if (error) {
      ...
      return <div className="mt-5">
        {!enough ? <ActionButton handleClick={() => executeAllowance()}>Allow MicroStorage to
              spend {coinNames[coin]}</ActionButton>
            : <ActionButton additionalClasses="mt-3"
                            handleClick={() => execute()}>Pay {price} {coinNames[coin]}</ActionButton>}
      </div>;
    }
  };
        ...
        <div>
          <TemplateSpec name="PAYMENT COIN">
            <select className="rounded-xl border border-dashed border-neutral-800 bg-inherit px-5 py-1 outline-0"
              ...
              {Object.entries(coinNames).map(([key, value]) => {
          ...
          <TemplateSpec name="PRICE PER DAY">{Math.round((0.01 + 0.001 * size) * 1000) / 1000} USD per
            day</TemplateSpec>
          ...
          <TemplateSpec name="TOTAL IN COINS">{price} {coinNames[coin]}</TemplateSpec>
          {
            writeButton()
          }
        </div>
